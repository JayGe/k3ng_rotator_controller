...
#ifdef FEATURE_AZ_POSITION_INCREMENTAL_ENCODER_Z_PIN  //ADDED
...
#ifdef FEATURE_EL_POSITION_INCREMENTAL_ENCODER_Z_PIN  //ADDED
  ...
  while (!stop_looping2) {
    ...
    while (!stop_looping1) {
      ...
      if (get_line_result == 0) {
        if (((pass == 0) && (strcmp(tle_line1, satellite_to_find) == 0)) || ((pass == 1) && (strcmp(tle_line1, alternate_satellite_search_string) == 0)) || ((pass == 2) && (strncmp(tle_line1, satellite_to_find, 4) == 0))) {
// if (verbose == _VERBOSE_){
//   control_port->print(F("Loading satellite "));
...
void output_debug() {
  ...
  if (((millis() - last_debug_output_time) >= ((int)periodic_debug_dump_time_seconds * 1000)) && (debug_mode)) {
    ...
    switch (autocorrect_state_az) {
      ...
      case AUTOCORRECT_WAITING_AZ:
        debug.print("AUTOCORRECT_WAITING_AZ: ");
        debug.print(autocorrect_az, 2);
        break;
      case AUTOCORRECT_WATCHING_AZ:
        debug.print("AUTOCORRECT_WATCHING_AZ: ");
        debug.print(autocorrect_az, 2);
        break;
    }
    ...
    switch (autocorrect_state_el) {
      case AUTOCORRECT_INACTIVE: debug.print("INACTIVE"); break;
      case AUTOCORRECT_WAITING_EL:
        debug.print("AUTOCORRECT_WAITING_EL: ");
        debug.print(autocorrect_el, 2);
        break;
      case AUTOCORRECT_WATCHING_EL:
        debug.print("AUTOCORRECT_WATCHING_EL: ");
        debug.print(autocorrect_el, 2);
        break;
    }
#endif  //FEATURE_ELEVATION_CONTROL
...
void update_el_variable_outputs(byte speed_voltage) {
  ...
  if (((el_state == SLOW_START_DOWN) || (el_state == NORMAL_DOWN) || (el_state == SLOW_DOWN_DOWN) || (el_state == TIMED_SLOW_DOWN_DOWN) || (el_state == SLOW_START_UP) || (el_state == NORMAL_UP) || (el_state == SLOW_DOWN_UP) || (el_state == TIMED_SLOW_DOWN_UP)) && (rotate_up_down_pwm)) {
#ifdef DEBUG_VARIABLE_OUTPUTS
      ...
      if (rotation_action == ACTIVATE) {
        ...
        if (az_slowstart_active) {
          ...
          if (rotate_ccw_pwm) {
            ...
            digitalWriteEnhanced(rotate_ccw_pwm, LOW);
          }
        ...
        if (rotate_cw_pwm) {
          analogWriteEnhanced(rotate_cw_pwm, 0);
          digitalWriteEnhanced(rotate_cw_pwm, LOW);
        }
        if (rotate_cw_ccw_pwm) {
          analogWriteEnhanced(rotate_cw_ccw_pwm, 0);
        }
      ...
      if (rotation_action == ACTIVATE) {
        ...
        if (el_slowstart_active) {
          ...
          if (rotate_down_pwm) {
            ...
            digitalWriteEnhanced(rotate_down_pwm, LOW);
          }
        ...
        if (rotate_up_pwm) {
          analogWriteEnhanced(rotate_up_pwm, 0);
          digitalWriteEnhanced(rotate_up_pwm, LOW);
        }
        if (rotate_up_down_pwm) {
          analogWriteEnhanced(rotate_up_down_pwm, 0);
        }
...
void service_rotation() {
  ...
  if (((az_state == NORMAL_CW) || (az_state == NORMAL_CCW)) && (az_request_queue_state == IN_PROGRESS_TO_TARGET) && az_slowdown_active && (abs((target_raw_azimuth - raw_azimuth)) <= SLOW_DOWN_BEFORE_TARGET_AZ)) {
  ...
  // normal -------------------------------------------------------------------------------------------------------------------
  // if slow down is enabled, see if we're ready to go into slowdown
  if (((el_state == NORMAL_UP) || (el_state == SLOW_START_UP) || (el_state == NORMAL_DOWN) || (el_state == SLOW_START_DOWN)) && (el_request_queue_state == IN_PROGRESS_TO_TARGET) && el_slowdown_active && (abs((target_elevation - elevation)) <= SLOW_DOWN_BEFORE_TARGET_EL)) {
...
void service_request_queue() {
  ...
  float work_target_raw_azimuth = 0;  // ADDED int to float
...
byte process_backslash_command(byte input_buffer[], int input_buffer_index, byte source_port, byte include_response_code, char *return_string, byte input_source) {
  ...
  switch (input_buffer[1]) {
    ...
    case 'A':  // \Ax[x][x] - manually set azimuth
      ...
      //  ---- hack 6-31-2023 by n5tm to calibrate to .1 degrees Az  This code was coppied from the #else def below. //ADDED
      place_multiplier = 1;
      ...
      if ((new_azimuth >= 0) && (new_azimuth < 360)) {
        ...
        dtostrf(new_azimuth, 0, 1, temp_string);  //ADDED
        strcat(return_string, temp_string);
      } else {
    ...
    case 'B':  // \Bx[x][x] - manually set elevation
      ...
      //  ---- hack 6-31-2023 by n5tm to calibrate to .1 degrees El  This code was coppied from the #else def below. ADDED
      place_multiplier = 1;
      for (int x = input_buffer_index - 1; x > 1; x--) {
        if (char(input_buffer[x]) != '.') {
          tempfloat += (input_buffer[x] - 48) * place_multiplier;
      ...
      //  ---- end - hack by n5tm to calibrate to .1 degrees El  This code was coppied from the #else def below.
      ...
      if ((new_elevation >= 0) && (new_elevation <= 180)) {
        ...
        dtostrf(new_elevation, 0, 1, temp_string);  //ADDED
        strcat(return_string, temp_string);
      } else {
    ...
    case 'O':  // set clock UTC time
      ...
      if ((temp_year > 2020) && (temp_year < 2070) && (temp_month > 0) && (temp_month < 13) && (temp_day > 0) && (temp_day < 32) && (temp_hour >= 0) && (temp_hour < 24) && (temp_minute >= 0) && (temp_minute < 60) && (temp_second >= 0) && (temp_second < 60) && ((input_buffer_index == 14) || (input_buffer_index == 16))) {
#if defined(OPTION_USE_OLD_TIME_CODE)
    ...
    case '#':
      ...
      while (((millis() - tle_upload_start_time) < 20000) && ((got_return < 2) || (tle_serial_buffer_in_pointer != tle_serial_buffer_out_pointer))) {
        ...
        while ((control_port->available()) && ((tle_serial_buffer_in_pointer + 1) != tle_serial_buffer_out_pointer) && (!((tle_serial_buffer_in_pointer == 2000) && (tle_serial_buffer_out_pointer == 0)))) {
...
//ZZZZZZ
      ...
      if (input_buffer[2] == 'T') {
        if ((input_buffer[3] == 'S') || (input_buffer[3] == 'U') || (input_buffer[3] == 'M') || (input_buffer[3] == 'X') || (input_buffer[3] == 'Y') || (input_buffer[3] == 'Z')) {
          temp_int = 0;
    ...
    if (slave_command_buffer_index == 4) {
      if ((slave_command_buffer[0] == 'S') && (slave_command_buffer[1] == 'W')) {  // Serial Write command
        switch (slave_command_buffer[2]) {
          case '0':
            control_port->write(slave_command_buffer[3]);
            command_good = 1;
            break;
#if defined(FEATURE_MASTER_WITH_SERIAL_SLAVE)
          case '1':
            REMOTE_PORT.write(slave_command_buffer[3]);
            command_good = 1;
            break;
#endif
        }
...
void service_moon_tracking() {
  ...
  if ((moon_tracking_active) && ((millis() - last_check) > configuration.tracking_moon_check_frequency_ms)) {
    ...
    if ((moon_visible) && ((millis() - last_tracking_submit_request) >= configuration.tracking_moon_minimum_rotation_interval_ms)
        && ((abs(azimuth - moon_azimuth) > configuration.tracking_moon_degrees_difference_threshold) || (abs(elevation - moon_elevation) > configuration.tracking_moon_degrees_difference_threshold))) {
...
void service_sun_tracking() {
  ...
  if ((sun_tracking_active) && ((millis() - last_check) > configuration.tracking_sun_check_frequency_ms)) {
    ...
    if ((sun_visible) && ((millis() - last_tracking_submit_request) >= configuration.tracking_sun_minimum_rotation_interval_ms)
        && ((abs(azimuth - sun_azimuth) > configuration.tracking_sun_degrees_difference_threshold) || (abs(elevation - sun_elevation) > configuration.tracking_sun_degrees_difference_threshold))) {
...
#define CALC_SEQUENTIAL_INTELLIGENT 1
  ...
  if ((satellite_tracking_active) && ((millis() - last_tracking_check) > configuration.tracking_sat_check_frequency_ms)) {
    ...
    if (((satellite[current_satellite_position_in_array].status & 1) == 1) && ((millis() - last_tracking_submit_request) >= configuration.tracking_sat_minimum_rotation_interval_ms)
        && ((abs(azimuth - current_satellite_azimuth) > configuration.tracking_sat_degrees_difference_threshold) || (abs(elevation - current_satellite_elevation) > configuration.tracking_sat_degrees_difference_threshold))) {
...
// REMOTE_UNIT_GS_COMMAND:
    ...
    if ((remote_unit_port_buffer_index == 13) && (remote_unit_port_buffer[0] == 'A') && (remote_unit_port_buffer[1] == 'Z') && (is_ascii_number(remote_unit_port_buffer[2])) && (is_ascii_number(remote_unit_port_buffer[3])) && (is_ascii_number(remote_unit_port_buffer[4])) && (is_ascii_number(remote_unit_port_buffer[6])) && (is_ascii_number(remote_unit_port_buffer[7])) && (is_ascii_number(remote_unit_port_buffer[8])) && (is_ascii_number(remote_unit_port_buffer[9])) && (is_ascii_number(remote_unit_port_buffer[10])) && (is_ascii_number(remote_unit_port_buffer[11]))) {
    ...
    if ((remote_unit_port_buffer_index == 14) && (remote_unit_port_buffer[0] == 'E') && (remote_unit_port_buffer[1] == 'L') && (is_ascii_number(remote_unit_port_buffer[3])) && (is_ascii_number(remote_unit_port_buffer[4])) && (is_ascii_number(remote_unit_port_buffer[5])) && (is_ascii_number(remote_unit_port_buffer[7])) && (is_ascii_number(remote_unit_port_buffer[8])) && (is_ascii_number(remote_unit_port_buffer[9])) && (is_ascii_number(remote_unit_port_buffer[10])) && (is_ascii_number(remote_unit_port_buffer[11])) && (is_ascii_number(remote_unit_port_buffer[12]))) {
